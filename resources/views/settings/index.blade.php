            ...
            <tbody>
                @forelse($users as $user)
                    <tr style="border-bottom:1px solid var(--bordersoft)">
                        ...
                        <td style="padding:12px 14px;font-family:var(--mono);font-size:11px;color:var(--ink3)">{{ $user->created_at->format('Y-m-d H:i') }}</td>
                        <td style="padding:12px 14px;text-align:center">
                            <form method="POST" action="{{ route('settings.delete-user', $user->id) }}" style="margin:0;display:inline" onsubmit="return confirm('Delete user {{ $user->username }}?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" style="color:var(--danger);background:none;border:none;cursor:pointer;font-weight:700;font-size:12px">Delete</button>
                            </form>
                        </td>
                    </tr>
                @empty
